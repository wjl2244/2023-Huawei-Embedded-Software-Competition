...
class Env():
    ...
    def step(self, action, i):
        ...
        if radius  == 0:
            ...
            reward = - x_r ** 2 * 0.0001 - (radius - 55)** 2 * 0.05 # - i * 0.001
        if abs(x_r) < 25 and abs(radius - 55) < 10:
            done = True
            reward = 100
        else:
            done = False
        reward_forward = action[0]/abs(action[0]) + action[1]/abs(action[1])
        reward = reward + 5 * reward_forward
        ...
        # reward = reward - 1.0 * action[0]**2 - 1.0 * action[1]**2
    ...
    def get_image(self):
        ...
        if len(cnts) > 0:
            ...
            # print(x,y,radius)
            return x-width/2, y-height*2/3, radius
        else:
            return width*1.1, height*1.1, 0
...
class PolicyNet(nn.Module):
    ...
    def action(self, state):
        ...
        # print('state:{},\t action:{}'.format(state.detach().cpu().numpy(), action))
        return action
...
def main(env, agent, Episode, batch_size):
    ...
    old_buffer_len = agent.buffer.buffer_len()
    for episode in range(Episode):
        ...
        for i in range(50):
            ...
            action_in = [0,0]
            for j in range(env.dim_action):
                if action[j] < action_range[0]:
                    action_in[j] = action_range[0]
                elif action[j] > action_range[1]:
                    action_in[j] = action_range[1]
                else:
                    action_in[j] = action[j]
        ...
        print("episode:{}, Return:{}, buffer_capacity:{}".format(episode, score, agent.buffer.buffer_len()-old_buffer_len))
        # print("----------------------------------------------")
        Return.append(score)
        score = 0
    env.close()
    ...
    torch.save(agent.policy_net, "policy_net.pkl")
    torch.save(agent.value_net, "value_net.pkl")
    torch.save(agent.target_value_net, "target_value_net.pkl")
    torch.save(agent.q1_net, "q1_net.pkl")
    torch.save(agent.q2_net, "q2_net.pkl")
